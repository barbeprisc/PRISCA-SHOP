...
function hill_top_bar(){
	...
	require_once( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/top/contact_info.php' );
	require( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/top/social_icon.php' );
	echo '</div>';
	echo '</div>';
	echo '</div>';
}
...
function hill_center_header_before(){
	...
	if(get_theme_mod('hill_header_layout',$hill_global['hill_header_layout'])=='header1'){
		...
		require_once( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/header/cart.php' );
		require_once( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/header/search.php' );
		require_once( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/header/call_button.php' );
		echo '</div>';
		echo '</div>';
	}
}
...
function hill_footer_scroll_button(){
	...
	if(get_theme_mod('hill_display_scroll_button',$hill_global['hill_display_scroll_button'])==true){
		require_once( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/scroll_button/scroll_button.php' );
	}
}
//breadcrumb
...
function hill_section_body(){
	...
	if ( !is_front_page()){
		if(get_theme_mod('hill_display_breadcrumb_section',$hill_global['hill_display_breadcrumb_section'])==true){
			require_once( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/breadcrumb/breadcrumb.php' );
		}
	}
	echo '</div>';
}
	...
	if ( is_front_page() ){
		...
		foreach ( $hill_front_page_order as $val_hill_front_page_order ) {
			if($val_hill_front_page_order!='blog'){
				require_once( HILL_THEMEOPTION_DIR_PATH.'themes/comman/front/frontpage/'.$val_hill_front_page_order.'.php' );
			}
		}
